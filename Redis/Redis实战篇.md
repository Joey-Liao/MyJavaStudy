...
当用户发起请求时，会访问我们像tomcat注册的端口，任何程序想要运行，都需要**有一个线程对当前端口号进行监听**，tomcat也不例外，当监听线程知道用户想要和tomcat连接连接时，那会由监听线程创建socket连接，socket都是成对出现的，用户通过socket像互相传递数据，当tomcat端的socket接受到数据后，此时**监听线程会从tomcat的线程池中取出一个线程执行用户请求**，在我们的服务部署到tomcat后，线程会找到用户想要访问的工程，然后用这个线程转发到工程中的controller，service，dao中，并且访问对应的DB，在用户执行完请求后，再统一返回，再找到tomcat端的socket，再将数据写回到用户端的socket，完成请求和响应

通过以上讲解，我们可以得知 每个用户其实对应都是**去找tomcat线程池中的一个线程来完成工作的， 使用完成后再进行回收**，既然每个请求都是独立的，所以在每个用户去访问我们的工程时，我们可以使用**threadlocal**来做到线程隔离，每个线程操作自己的一份数据
...
![image-20230418211955126](D:\Typora\workspace\Redis\Redis实战篇.assets\image-20230418211955126.png)
...
缓存穿透 ：缓存穿透是指客户端请求的**数据在缓存中和数据库中都不存在**，这样缓存永远不会生效，这些请求都会打到数据库。
...
缓存击穿问题也叫**热点Key问题**，就是一个**被高并发访问**并且**缓存重建业务较复杂的key**突然失效了，无数的请求访问会在瞬间给数据库带来巨大的冲击。
...
![image-20230423213338096](D:\Typora\workspace\Redis\Redis实战篇.assets\image-20230423213338096.png)
...
### ![image-20230505153618520](D:\Typora\workspace\Redis\Redis实战篇.assets\image-20230505153618520.png)
...
2、我们需要找到与上一次查询相同的查询个数(因为最大值是大于等于，所以要跳过)作为偏移量，下次查询时，跳过这些查询过的数据，拿到我们需要的数据

综上：我们的请求参数中就需要携带 lastId：上一次查询的最小时间戳 和偏移量这两个参数。

这两个参数第一次会由前端来指定，以后的查询就根据后台结果作为条件，再次传递到后台。

![image-20230508190500487](D:\Typora\workspace\Redis\Redis实战篇.assets\image-20230508190500487.png)
